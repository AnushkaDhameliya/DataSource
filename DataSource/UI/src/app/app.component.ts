...
export class AppComponent implements OnInit {
  ...
  constructor(private sanitizer: DomSanitizer, public http: HttpClient, public apiService : ApiService) {
  ...
  public registerEvents(editor: any): void {
    ...
    editor.on("nodeSelected", id => {
      ...
      this.fetchLoadData();
      if (editor.container.querySelector('#node-'+id+' .title-box .iconInfo').innerHTML.includes('Merge')) {
      ...
      } else if (editor.container.querySelector('#node-'+id+' .title-box .iconInfo').innerHTML.includes('.csv') || editor.container.querySelector('#node-'+id+' .title-box .iconInfo').innerHTML.includes('.xlsx')) {
        this.actionEvent = 'Detail Information of ' + editor.container.querySelector('#node-'+id+' .title-box .iconInfo').innerHTML;
        this.popupFlagAction = true;
        this.actionEventData = this.selectedData[parseInt(id) - 1];
        console.log(this.selectedData);
      } else if (!editor.container.querySelector('#node-'+id+' .title-box .iconInfo').innerHTML.includes('Output')) {
  ...
  public showOutput() {
    ...
    var val = {
      "ActionName" : "Merge",
      "fileList" : [
          "C:/Users/anush/Desktop/Virtual Desktop/Data Analytics Files/data.xlsx",
          "C:/Users/anush/Desktop/Virtual Desktop/Data Analytics Files/dataSheet2.xlsx",
          "C:/Users/anush/Desktop/Virtual Desktop/Data Analytics Files/dataSheet3.xlsx"
          ],
      "mergeCoulmnName" : "Muni Code",
      "sortColumnNames" : ["Block"],
      "projectColumns" : ["Block","Muni Code"],
      "outputFileName" : "data1"
    }

    this.apiService.performAction(val).subscribe(res=> alert(res.toString()));
